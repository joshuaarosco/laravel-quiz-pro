        ...
        <div class="table-responsive">
            <table class="table table-hover table-condensed" id="condensedTable">
                <tbody>
                    ...
                    <tr>
                        <td>{{$index+1}}</td>
                        <td class="v-align-middle">{{$info->quiz->title?:'---'}}</td>
                        <td class="v-align-middle">{{Str::upper(str_replace('_',' ',$info->status))?:'---'}}</td>
                        <td class="v-align-middle">{{__('difficulty.'.$info->quiz->difficulty)}}</td>
                        <td class="v-align-middle">{{$info->quiz->time_limit?:'--:--'}} minutes</td>
                        <td class="v-align-middle">{{$info->user->name}}</td>
                        <td class="text-right">
                            <a
                                class="btn btn-default btn-rounded btn-md btn-edit"
                                title="Answer" href="{{route('backoffice.quiz.answer',['id' => $info->id])}}">
